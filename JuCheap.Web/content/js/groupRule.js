(function (root, factory) {
    ...
    var __PROTOTYPE__ = {
        _init: function () {
            ...
            this.code = '<li>' +
                        '<p class="authitem auth1 pr" data-id="1"><span><i class="iconfont">' + this.titleIcon + '</i>' + this.title + '</span><span style="float：left;padding-left: 50px;" data_name="allchk" onclick="AllSelected_True()"><i class="iconfont icon-checkbox"  >ㄨ</i>全不选</span><span style="float：left;padding-left: 50px;" data_name="allchk" onclick="AllSelected_False()"><i class="iconfont icon-checkbox"  >ㄨ</i>全选<span></p>' +
        ...
        _render: function () {
            ...
            $.each(this.data, function (i, data) {
                var _data = data;
                var icon = data.checked ? "&#x3127;" : "&#x3128;";
                var _checked = data.checked ? "checked" : "";

                code += '<li class="auth-li" data-id="' + data.id + '">' +
                    '<p class="authitem auth2 pr">' +
                        '<span class="btn-toggle">' +
                            '<span>' +
                                '<i class="iconfont icon-checkbox ' + _checked + '"  mhref="' + data.id + '">' + icon + '</i>' +
                            '</span>' +
                            data.text +
                        '</span>' +
                ...
                $.each(data.childs, function (j, data) {
                    code += '<li>' +
                        '<p class="authitem auth3 pr" data-id="' + data.id + '">' +
                            '<span class="btn-toggle">' +
                                '<span>' +
                                    '<i class="iconfont icon-checkbox' + _checked + '" mhref="' + data.id + '" phref="' + _data.id + '">' + icon + '</i>' +
                                '</span>' +
                                data.text +
                            '</span>' +
                            '<i class="iconfont icon-right-down">&#x3125;</i>' +
        ...
        _handle: function () {
            ...
            $("span[data_name=allchk]").on('click', function () { });
        }
    };
